...
export const uploadProofFile = async (uid, taskId, file) => {
  const path = `proofs/${uid}/${taskId}_${Date.now()}_${file.name}`;
  const fileRef = ref(storage, path);
  await uploadBytes(fileRef, file);
  return getDownloadURL(fileRef);
};

export const submitTask = (uid, taskId, note, fileUrl) =>
  addDoc(collection(db, "submissions"), { uid, taskId, note: note || "", fileUrl: fileUrl || "", status: "pending", ts: new Date().toISOString() });
