...
export const submitTask = (uid, taskId) =>
  addDoc(collection(db, "submissions"), { uid, taskId, status: "pending", ts: new Date().toISOString() });

export const decideSubmission = async (submissionId, decision, reward, uid) => {
  await updateDoc(doc(db, "submissions", submissionId), { status: decision });
