...
import {
  ...
  watchWallet, watchVideos, addVideo, deleteVideo, submitKyc,
} from "./data.js";

const PAY_NUMBERS = [
  { name: "বিকাশ", number: "01700-000000" },
  { name: "নগদ", number: "01800-000000" },
  { name: "রকেট", number: "01900-000000" },
];
...
function KycScreen({ uid, flash }) {
  ...
  const [form, setForm] = useState({ age: "", education: "", address: "", nid: "", captcha: "" });
  const [busy, setBusy] = useState(false);

  const submit = async () => {
    if (!form.age.trim() || !form.education.trim() || !form.address.trim()) {
      return flash("সব ঘর পূরণ করুন");
    }
    ...
    try {
      await submitKyc(uid, {
        age: form.age.trim(), education: form.education.trim(),
        address: form.address.trim(), nid: form.nid.trim(),
      });
    } catch {
      flash("সমস্যা হয়েছে, আবার চেষ্টা করুন");
    }
    setBusy(false);
  };
  ...
  return (
    <div className="px-5 pt-8">
      ...
      <Field label="বয়স" value={form.age} onChange={(v) => setForm({ ...form, age: v })} placeholder="যেমন: ২২" />
...
function ProfileTab({ profile, sub, submissions }) {
  ...
  return (
    <div className="px-5 pt-6">
      ...
      <button onClick={() => setShowTerms(true)} className="w-full text-sm text-[#16233F] mt-4 py-2 border border-gray-300 rounded-xl">
        শর্তাবলী ও স্বচ্ছতা
      </button>
